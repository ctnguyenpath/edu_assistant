...
const HomePage = ({ onPlayVideo, videos }) => {
  return (
    <div className="p-8 overflow-y-auto h-full bg-gray-50 dark:bg-[#0A0A0A] transition-colors duration-300">
      
      {/* --- HEADER --- */}
      <div className="mb-10">
        <h1 className="text-4xl font-medium text-gray-900 dark:text-gray-100 mb-2 transition-colors duration-300">
          Welcome back
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {videos.map((clip) => (
          <div 
            ...
            className="group bg-white dark:bg-[#1E1F20] border border-gray-200 dark:border-transparent rounded-2xl overflow-hidden hover:ring-2 hover:ring-blue-500/30 dark:hover:ring-gray-600 transition-all cursor-pointer shadow-sm dark:shadow-none"
          >
            {/* Thumbnail Area */}
            <div className="h-40 bg-gray-200 dark:bg-gray-800 relative flex items-center justify-center transition-colors duration-300">
              {/* Gradient Overlay to make the Play button pop on light thumbnails */}
