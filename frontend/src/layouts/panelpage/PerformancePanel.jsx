...
const PerformancePanel = ({ isOpen, onToggle, activeLesson, currentPerf, getScoreColor }) => {
  return (
    <div className="border-t border-[#333] bg-[#131314] flex flex-col shrink-0">
      <div 
        onClick={onToggle}
        className="p-4 flex items-center justify-between cursor-pointer hover:bg-[#1E1F20] transition-colors select-none"
      >
         <h2 className="text-gray-100 font-bold text-xs tracking-widest uppercase flex items-center gap-2">
      ...
      {isOpen && (
        <div className="p-4 pt-0 space-y-6 overflow-y-auto max-h-[40vh] scrollbar-thin scrollbar-thumb-[#333]">
           {/* Selected Module Summary */}
           <div className="bg-[#1E1F20] p-4 rounded-xl border border-[#333] shadow-inner">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Current Focus</p>
              <h3 className="text-white font-bold text-sm mb-3 truncate">{activeLesson?.topic || 'No Module Selected'}</h3>
              <div className="flex items-baseline gap-2">
                 <span className="text-4xl font-black text-white">{currentPerf?.score_value ?? '--'}</span>
                 <span className="text-gray-600 font-bold text-sm">/ 10</span>
              </div>
           ...
           {currentPerf?.score_value !== null && currentPerf?.score_value < 5 && (
             <div className="bg-red-900/10 border border-red-500/20 p-3 rounded-xl flex gap-3 animate-pulse">
                ...
                <p className="text-[10px] text-red-200 leading-tight font-medium">Below proficiency threshold. Re-take quizzes.</p>
             </div>
           )}
           ...
           <div className="space-y-3">
              ...
              <div className="w-full bg-[#1E1F20] h-2 rounded-full overflow-hidden border border-[#333]">
